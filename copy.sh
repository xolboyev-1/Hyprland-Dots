...
while true; do
  echo "${NOTE} ${SKY_BLUE} By default, Dots are configured for 1440p or 2k."
...
if [ -d "$BACKUP_DIR_PATH" ]; then
  ...
  print_color $WARNING "
    ...
    "
  echo -e "${MAGENTA}Kindly Visit and check Hyprland-Dots GitHub page for the history of commits.${RESET}"
...
while true; do
  ...
  [Yy])
    ...
    if git clone "https://github.com/xolboyev-1/wallpapers.git"; then
...
printf "\n%.0s" {1..2}
printf "${OK} GREAT! Hyprland-Dots is now Loaded & Ready !!! "
printf "\n%.0s" {1..1}
printf "${INFO} However, it is ${MAGENTA}HIGHLY SUGGESTED${RESET} to logout and re-login or better reboot to avoid any issues"
printf "\n%.0s" {1..1}
printf "${SKY_BLUE}Thank you${RESET} for using ${MAGENTA}our Hyprland Configuration${RESET}... ${YELLOW}ENJOY!!!${RESET}"
printf "\n%.0s" {1..3}
