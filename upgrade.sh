...
echo "${SKY_BLUE}This script is meant to manually upgrade your Hyprland Dots${RESET}"
...
if version_gt "$latest_version" "$stored_version"; then
  ...
  echo "$OK 👌 No upgrade found. The installed version ${MAGENTA}($stored_version)${RESET} is up to date with the Hyprland-Dots version ${YELLOW}($latest_version)${RESET}" 2>&1 | tee -a "$LOG"
fi
